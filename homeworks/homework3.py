...
def main(cfg):
    src_image = cv2.imread(
        os.path.join(os.path.dirname(__file__), "..", cfg["input_image_path"]),
    ...
    opencv_dst_image = cv2.bilateralFilter(
        src=src_image, sigmaSpace=sigma_space, sigmaColor=sigma_color, d=-1
    )
    ...
    numpy_dst_image = bilateral_filter(
        src_image=src_image, sigma_space=sigma_space, sigma_color=sigma_color
    )
