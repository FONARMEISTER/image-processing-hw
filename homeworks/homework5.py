...
def find_matches_with_cross_check(D, T):
    h = D.shape[0]
    argmin_12 = np.argmin(D, axis=1)
    min_12 = np.min(D, axis=1)
    argmin_21 = np.argmin(D, axis=0)

    iota_list = np.arange(h)
    is_mutual = argmin_21[argmin_12] == iota_list
    is_mutual = np.where(min_12 < T, is_mutual, False)
    return np.column_stack((argmin_12[is_mutual], iota_list[is_mutual]))


def main(cfg):
    net = SuperPointNet()
    net.load_state_dict(torch.load(cfg["superpoint_weights"]))

    img1 = cv2.imread(cfg["input_image_1"])
    img2 = cv2.imread(cfg["input_image_2"])
    pts1, des1 = net(img1)
    pts2, des2 = net(img2)
    ...
    pairs = find_matches_with_cross_check(D, T)
    output_image = np.concatenate((img1, img2), axis=1)
    W = img1.shape[1]
    for i, j in pairs:
        pt1 = pts1[i]
        pt2 = (pts2[j][0] + W, pts2[j][1])
        cv2.circle(output_image, pt1, 3, (255, 0, 255), -1)
        cv2.circle(output_image, pt2, 3, (255, 0, 255), -1)
